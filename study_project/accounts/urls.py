...
from study_project.accounts import views as userena_views
from study_project.accounts import settings as userena_settings
from study_project.accounts.compat import auth_views_compat_quirks, password_reset_uid_kwarg
...
urlpatterns = patterns('',
    ...
    # View results
    url(r'^(?P<username>[\.\w-]+)/results/$',
       userena_views.results_list,
       name='userena_results'),

    # View blog
    url(r'^(?P<username>[\.\w-]+)/blog/$',
       userena_views.view_blog,
       name='userena_blog'),

    # Add note to the blog
    url(r'^(?P<username>[\.\w-]+)/add_note/$',
       userena_views.add_note,
       name='userena_add_note'),


    # View prepods list
    url(r'^(?P<username>[\.\w-]+)/prepods/$',
       userena_views.prepods,
       name='userena_prepods'),

    # Send message to the prepod
    url(r'^(?P<username>[\.\w-]+)/(?P<prepod_id>\d+)/$',
       userena_views.send_prepod_msg,
       name='userena_send_prepod_msg'),
)
