...
from study_project.lection_part.views import get_content_paged_list

from userena.models import UserenaSignup
from userena.decorators import secure_required
from userena.backends import UserenaAuthenticationBackend
from study_project.accounts.utils import signin_redirect, get_profile_model, get_user_model
from userena import signals as userena_signals
from study_project.accounts import settings as userena_settings

from guardian.decorators import permission_required_or_403

from study_project.practic_part.models import PracticalLessonResult, PracticalLesson, Answer, CourseResult, Course

from study_project.accounts.models import Note, Message, Prepod
from study_project.accounts.forms import AddNoteForm, EditProfileForm, MessageForm 


from study_project.accounts.models import Note
from study_project.accounts.forms import AddNoteForm
...
def results_list(request, username):
    ...
    if username != user.username:
        return render(request, 'userena/user_results_list.html', {'error': True})
    else:
        course_results = {}
        course_res = CourseResult.objects.filter(user = user)
        for c in course_res:
            results = {}
            res = PracticalLessonResult.objects.filter(practical_course = c)
            for r in res:
                results.update({ r : PracticalLesson.objects.get(id=r.practical_lesson.id)})
            course_results.update({ c : [Course.objects.get(id = c.practical_course.id), results]})

        return render(request, 'userena/user_results_list.html', {'course_results': course_results, 
                                                                  'user' : user, 'error': False})

        return render(request, 'userena/user_results_list.html', {'course_results': course_results, 'user' : user, 'error': False})
...
def view_blog(request, username):
    ...
    note_list = Note.objects.filter(user=cur_user)

    return render(request, 'userena/user_blog.html', {'note_list': note_list, 
                                                      'cur_user' : cur_user, 'my_blog': my_blog, 'user' : user})

    return render(request, 'userena/user_blog.html', {'note_list': note_list, 'cur_user' : cur_user, 'my_blog': my_blog, 'user' : user})
...
def add_note(request, username):
    ...
    if username != user.username:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                note = Note(user=cur_user, name=form.cleaned_data['name'], 
        ...
        return render(request, 'userena/user_add_note.html', {'form': form, 'error': False, 
                                                              'cur_user' : cur_user, 'user' : user})


def prepods(request, username):
    user = request.user
    if username != user.username:
        my_prep = False
    else: my_prep = True
    ...
    cur_user = get_object_or_404(User, username = username)
    prepod_list = cur_user.my_profile.prepod.all()
    return render(request, 'userena/user_prepods.html', {'prepod_list': prepod_list, 'cur_user' : cur_user, 
                                                         'my_prep': my_prep, 'user' : user})


def send_prepod_msg(request, username, prepod_id):
    user = request.user
    if username != user.username:
        return render(request, 'userena/user_send_msg.html', {'error': True, 'cur_user' : user})
    else:
        cur_user = get_object_or_404(User, username = username)
        prep = get_object_or_404(Prepod, id = prepod_id)

        if request.method == 'POST':
            form = MessageForm(request.POST)
            if form.is_valid():
                message = Message(user=cur_user, prep=prep, from_to = False,
                ...
                redir = '/accounts/' + username + '/prepods/'
                return HttpResponseRedirect(redir) # Redirect after POST
        else:
            form = MessageForm() # An unbound form

        message_list = Message.objects.filter(user=cur_user, prep=prep)
        messages = get_content_paged_list(message_list, request)
        return render(request, 'userena/user_send_msg.html', {'form': form, 'error': False, 
                                                              'cur_user' : cur_user, 'user' : user, 
                                                              'prep' : prep, 'message_list' : messages})

        return render(request, 'userena/user_add_note.html', {'form': form, 'error': False, 'cur_user' : cur_user, 'user' : user})
